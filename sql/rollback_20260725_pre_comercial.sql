-- ============================================================
-- ROLLBACK: placar() como estava em 25/07/2026, ANTES da
-- integracao com o RD CRM (tabela negocios + coluna Comercial).
-- Tag correspondente no repo: marco-pre-coluna-comercial
-- Backups no banco:
--   _backup_funcoes ids 3,4,5 (insere_conversao, placar,
--   sync_campanhas_monday, motivo "marco-pre-coluna-comercial")
--   _backup_placar_snapshot periodo "2026-07-23 a 2026-07-23"
--   (numeros validados na auditoria RD/CRM de 25/07)
-- Para reverter: rodar este arquivo inteiro no SQL Editor.
-- ============================================================
...
AS $function$
  ...
  -- dentro do periodo consultado. O canal atribuido e o da PRIMEIRA
  -- conversao da pessoa no periodo, para que as colunas de canal somem
  -- exatamente o total de leads.
  with base as (
